...
class Pipeline:
    ...
    def _setup_downloaders(self) -> List:
        """Setup default downloaders."""
        downloaders = []
        
        hf_datasets = {
            "MuMiN-PT": {
                "ds": "ju-resplande/portuguese-fact-checking",
                "subset": "MuMiN-PT",
                "source_type": "news+social",
                "source_description": "Coleção PT do MuMiN processada",
            },
        }
        
        for name, spec in hf_datasets.items():
            downloaders.append(
                HuggingFaceDownloader(spec["ds"], spec["subset"], name)
            )
    ...
    def download(self) -> List[str]:
        ...
        for downloader in self.downloaders:
            ...
            print(f"Downloaded {len(paths)} files from {downloader.__class__.__name__}")
        return all_paths
    ...
    def normalize_and_merge(
        ...
        Args:
            local_files: Optional dict of local files to include
    ...
    def clean(self, input_path: Optional[str] = None) -> pd.DataFrame:
        ...
        Args:
            input_path: Path to input file (if None, uses merged dataset)
    ...
    def add_deduplication(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Add near-duplicate detection to DataFrame.
        
        Args:
            df: Input DataFrame with text_clean column
            
        Returns:
            DataFrame with near_duplicates column
        """
    ...
    def factcheck(self, input_path: Optional[str] = None, output_path: Optional[str] = None) -> str:
        ...
        Args:
            input_path: Path to input CSV (if None, uses clean dataset)
            output_path: Path to save output (if None, uses default)
            
        Returns:
            Path to fact-checked dataset
        """
